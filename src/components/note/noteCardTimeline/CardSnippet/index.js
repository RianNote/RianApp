import React, { Component } from 'react';
import css from './cardSnippet.css'

const makeTagToElement = (tagSet) => {
	if (!Array.isArray(tagSet)) {
		console.log('is Not Array')
		return []
	}
	return tagSet.map((data, index)=>{
		return <div key={index} className={css.tagInstance}>{data}</div>
	})
} 

const CardSnippet = ({title, preview, tag, thumbnail }) => {
	return (
		<div className={css.container}>
			<div className={css.head}>
				<p className={css.title}>{title}</p>
			</div>
			<div className={css.footer}>
				<p className={css.preview}>{preview}</p>
				<div className={css.tag}>
					<img className={css.tagImg} src='data:image/png;base64,' />
					{makeTagToElement(tag)}
				</div>
			</div>
		</div>
	)
}

export default CardSnippet